...
/**
 * The database is canonical for plan state, not anything cached locally.
 * If a case built from an earlier save of this same tank/window was later
 * abandoned, the Blend Case Manager's abandon_blend_case RPC reverts that
 * original blend_plans row back to status='proposed' rather than deleting
 * it. Without this check, re-saving the same plan would insert a brand
 * new row alongside the reverted one -- two "proposed" rows claiming the
 * same tank and window. Reusing (updating) the existing proposed row
 * instead keeps blend_plans as the single source of truth for "what's
 * proposed for this tank/window" rather than accumulating duplicates.
 */
async function findReusableProposedPlan(tankNo, tankLabel, windowStart) {
  const { data, error } = await supabase
    .from('blend_plans')
    .select('id, plan_code')
    .eq('status', 'proposed')
    .eq('window_start', windowStart)
    .or(tankNo ? `tank_no.eq.${tankNo},tank.eq.${tankLabel}` : `tank.eq.${tankLabel}`)
    .limit(1)
    .maybeSingle();
  if (error) {
    console.warn('[savePlanToSupabase] could not check for a reusable proposed plan, will insert a new row:', error);
    return null;
  }
  return data;
}
 ...
 * @returns {Promise<{success:boolean, rowsAdded?:number, rowsUpdated?:number, error?:string}>}
 */
export async function savePlanToSupabase({ terminalConfig, blends, startDate, blendTarget, notes }) {
  ...
  // Blends that reuse an existing proposed row (update) are resolved
  // first; only blends that need a brand new row go through plan-code
  // assignment, so week sequences aren't burned on updates.
  const toInsert = [];
  let rowsUpdated = 0;
  for (const b of blends) {
    const tankNo = tankAssetTagFor(b);
    const tankLabel = b.tankLabel || b.tankId;
    const windowStart = `${b.startDate} ${b.startTime}`;
    const existing = await findReusableProposedPlan(tankNo, tankLabel, windowStart);
    if (existing) {
      const { error } = await supabase.from('blend_plans').update(fieldsFor(b)).eq('id', existing.id);
      if (error) {
        console.error('[savePlanToSupabase] update of reused proposed plan failed:', error);
  ...
  if (!toInsert.length) {
    return { success: true, rowsAdded: 0, rowsUpdated };
  }

  const planCodes = await assignPlanCodes(toInsert);
  ...
  for (let i = 0; i < toInsert.length; i++) {
    ...
    while (attempt < 5) {
      const { error } = await supabase.from('blend_plans').insert({ plan_code: code, ...fieldsFor(toInsert[i]) });
      if (!error) {
        rowsAdded += 1;
        break;
      }
  ...
  return { success: true, rowsAdded, rowsUpdated };
}
